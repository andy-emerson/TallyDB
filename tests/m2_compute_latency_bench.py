...
Usage: m2_compute_latency_bench.py [libtallydb.so] [rows] [iters]
Defaults: target/debug/libtallydb.so, 20000 rows, 3 iterations (min
...
def load_library(path_arg):
    if path_arg:
        ...
        path = repo / "target" / "debug" / "libtallydb.so"
    if not path.exists():
        sys.exit(
